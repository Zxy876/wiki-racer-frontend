...
export const GameResult: React.FC<GameResultProps> = ({ game, onNewGame }) => {
  ...
  const pathLength = game.path ? game.path.length - 1 : 0 // 减去起点
    ...
    <div className={styles.container}>
      <div className={styles.resultHeader}>
        ...
        <div className={styles.gameInfo}>
          <span>从 <strong>{game.start}</strong> 到 <strong>{game.end}</strong></span>
        </div>
      </div>
      ...
      {hasPath ? (
        ...
        <div className={styles.noPathResult}>
          ...
          <h3>未找到路径</h3>
          <p>在指定的搜索深度内未找到从 <strong>{game.start}</strong> 到 <strong>{game.end}</strong> 的路径。</p>
          <p>请尝试：</p>
          <ul>
            <li>使用更常见的页面标题</li>
            <li>检查拼写是否正确</li>
